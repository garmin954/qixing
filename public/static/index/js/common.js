...
$(window).load(function (){ 
    ...
    $('.jdhelpnav li').mouseover(function (event) {
        $(this).siblings('li').attr('class', '');
        $(this).attr('class', 'current');
        var num = $(this).index();
        $('.tabnav ul').eq(num).siblings().hide();
        $('.tabnav ul').eq(num).show();
        $('.section6 .tabBdlc .lcrydymb').eq(num).siblings().hide();
        $('.section6 .tabBdlc .lcrydymb').eq(num).show();
    });

	    // 成功案例
    $(function  () {
        $('.section4 .byhbd li a').hover(function() {
            $(this).children('.jianjiebb').stop().css('height', '201px').slideDown(150);
        }, function() {
            $(this).children('.jianjiebb').stop().slideUp(150);;
        });
    })
	//当存在可溢出层时重新定义这些层的高度
