...
function get_addons_list()
{
    ...
    $addons_list = scandir(ADDONS_PATH);
    $info_list=[];
    foreach ($addons_list as $val){
        if(is_dir(ADDONS_PATH.$val)){
            if($val != '.' && $val!= '..'){
                //判断文件里是否存在插件类
                if(!is_file(ADDONS_PATH.$val.DS.ucfirst($val).'.php')){
                    continue;
                }
...
{
    $file = ADDONS_PATH.$config['name'].DS.$name;
    if(file_exists($file) && is_file($fiel)){
        return false;
    }
    $config = var_export($file,true);
    $content = <<<Yl
<?php
    return {$config}
Yl;
